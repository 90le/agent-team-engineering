...
python3 /opt/agent-team-factory-v0.6.0/tools/agent_team.py doctor
```
...
`v0.6.0`实现从 `0.2.0`、`0.3.0`、`0.4.0`、`0.5.0`、`0.5.1` 到 `0.6.0` 的显式迁移。以后每个目标版本必须单独声明可接受来源；能够读取旧实例不代表存在升级路径。
...
python3 tools/agent_team.py instance upgrade apply \
  ...
  --recovery /safe/recovery/instance-v0.4-before-v0.6.0
```
...
```bash
python3 tools/agent_team.py instance recovery-inspect \
  --bundle /safe/recovery/instance-v0.4-before-v0.6.0

python3 tools/agent_team.py instance rollback \
  --root /path/to/team-instance \
  --recovery /safe/recovery/instance-v0.4-before-v0.6.0 \
  --rescue /safe/recovery/instance-v0.6.0-before-rollback
```
