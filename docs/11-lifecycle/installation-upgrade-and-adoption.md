...
python3 tools/agent_team.py factory install \
  --output /opt/agent-team-factory-v0.8.1

python3 /opt/agent-team-factory-v0.8.1/tools/agent_team.py factory verify \
  --root /opt/agent-team-factory-v0.8.1

python3 /opt/agent-team-factory-v0.8.1/tools/agent_team.py doctor
```
...
`v0.8.1`实现从 `0.2.0`、`0.3.0`、`0.4.0`、`0.5.0`、`0.5.1`、`0.6.0`、`0.7.0`、`0.8.0` 到 `0.8.1` 的显式迁移。以后每个目标版本必须单独声明可接受来源；能够读取旧实例不代表存在升级路径。v0.7 到 v0.8 的 Factory 文件升级不会迁移旧运行批准；Native v0.8 导入规则另见 [v0.8 core 契约与 v0.7 导入](../15-upstream-independent/core-contracts-and-migration.md)。v0.8.0到v0.8.1是采用入口与生成上下文补丁，仍须走摘要绑定升级，不能手工改锁冒充完成。
...
python3 tools/agent_team.py instance upgrade apply \
  ...
  --recovery /safe/recovery/instance-v0.7-before-v0.8.1
```
...
```bash
python3 tools/agent_team.py instance recovery-inspect \
  --bundle /safe/recovery/instance-v0.7-before-v0.8.1

python3 tools/agent_team.py instance rollback \
  --root /path/to/team-instance \
  --recovery /safe/recovery/instance-v0.7-before-v0.8.1 \
  --rescue /safe/recovery/instance-v0.8.1-before-rollback
```
