...
python3 tools/agent_team.py factory install \
  --output /opt/agent-team-factory-v1.0.0

python3 /opt/agent-team-factory-v1.0.0/tools/agent_team.py factory verify \
  --root /opt/agent-team-factory-v1.0.0

python3 /opt/agent-team-factory-v1.0.0/tools/agent_team.py doctor
```
...
当前 `v1.0.0` 实现从 `0.2.0`、`0.3.0`、`0.4.0`、`0.5.0`、`0.5.1`、`0.6.0`、`0.7.0`、`0.8.0`、`0.8.1`、`0.9.0` 到 `1.0.0` 的显式迁移。以后每个目标版本必须单独声明可接受来源；能够读取旧实例不代表存在升级路径。历史 `v0.9.0` 已实现从 `0.2.0` 至 `0.8.1` 到 `0.9.0` 的迁移，这项已发布保证不因 v1 文档而改写。v0.7 到 v0.8 的 Factory 文件升级不会迁移旧运行批准；Native v0.8 导入规则另见 [v0.8 core 契约与 v0.7 导入](../15-upstream-independent/core-contracts-and-migration.md)。v0.8.1 到 v0.9.0 新增宿主能力、投影与安装契约，仍须走摘要绑定升级，不能手工改锁冒充完成。

`0.9.0 → 1.0.0` 的 Team Instance 迁移与 host projection 安装生命周期彼此独立。普通 instance upgrade 不会把 host installation plan `1.0.0` 升级为 `1.1.0`，也不会替 v0.9 install lock 补造完整 proposal ownership：尚未执行的 v0.9 host plan 必须重新 `plan → preview → confirm`；旧 lock 只能由 `host verify` 报告为 `LEGACY_UNBOUND` 并人工对账，自动破坏性卸载明确禁用。
...
python3 tools/agent_team.py instance upgrade apply \
  ...
  --recovery /safe/recovery/instance-v0.9.0-before-v1.0.0
```
...
```bash
python3 tools/agent_team.py instance recovery-inspect \
  --bundle /safe/recovery/instance-v0.9.0-before-v1.0.0

python3 tools/agent_team.py instance rollback \
  --root /path/to/team-instance \
  --recovery /safe/recovery/instance-v0.9.0-before-v1.0.0 \
  --rescue /safe/recovery/instance-v1.0.0-before-rollback
```
