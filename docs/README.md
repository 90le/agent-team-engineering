# 文档导航

- [项目定位](00-project-positioning/vision-and-scope.md)
- [项目宪法](01-principles/project-constitution.md)
- [参考架构](02-architecture/reference-architecture.md)
- [威胁模型](03-security/threat-model.md)
- [角色与权责](04-governance/roles-and-authority.md)
- [现有项目接入](05-adoption/existing-project-adoption.md)
- [发布与迁移](06-portability/release-and-migration.md)
- [验证与恢复](07-operations/verification-and-recovery.md)
- [实例生成与治理](08-factory/instance-lifecycle.md)
- [持久化与故障恢复](09-control-plane/persistence-and-recovery.md)
- [适配器SDK、隔离执行与认证批准](10-adapters/sdk-isolation-and-approval.md)
- [Factory安装、实例升级、恢复与项目接入](11-lifecycle/installation-upgrade-and-adoption.md)
- [跨AI冷启动接管验收](12-acceptance/cross-ai-takeover.md)
- [团队蓝图编译器与参考运行时](13-team-creator/blueprint-compiler-and-reference-runtime.md)
- [上下文优先Agent Team Kit](14-context-first/context-first-team-kit.md)
- [Codex、Claude、OpenClaw与通用AI安装](14-context-first/platform-installation.md)
- [v0.8供应商中立核心总入口](15-upstream-independent/README.md)
- [v0.8自有核心契约与v0.7迁移](15-upstream-independent/core-contracts-and-migration.md)
- [v1.0独立前端/后端写入者权威链](15-upstream-independent/independent-writer-topology.md)
- [v0.8可替换Adapter Port SDK](15-upstream-independent/adapter-port-sdk.md)
- [v0.8 Native Controller、恢复与一致性验收](15-upstream-independent/native-controller-and-conformance.md)
- [v0.8 Runner、SCM、Agent与外部平台边界](15-upstream-independent/runner-scm-and-agent-boundaries.md)
- [从生成团队到受控自动化](15-upstream-independent/adoption-and-integration.md)
- [v0.8.0发布验收](16-release/v0.8-acceptance.md)
- [v0.8.1引导采用发布验收](16-release/v0.8.1-acceptance.md)
- [v0.9.0宿主原生团队发布验收](16-release/v0.9-acceptance.md)
- [v1.0.0发布验收契约](16-release/v1.0-acceptance.md)
- [场景优先引导式采用](17-guided-adoption/README.md)
- [AI采用对话协议](17-guided-adoption/conversation-protocol.md)
- [采用方案契约](17-guided-adoption/plan-contract.md)
- [引导式采用FAQ](17-guided-adoption/faq.md)
- [宿主原生Agent Team Factory总入口](18-native-hosts/README.md)
- [宿主支持与证据矩阵](18-native-hosts/support-matrix.md)
- [宿主原生团队采用对话](18-native-hosts/conversation-workflow.md)
- [OpenClaw原生团队边界](18-native-hosts/openclaw.md)
- [Hermes Agent原生团队边界](18-native-hosts/hermes-agent.md)
- [Multica实验性方案](18-native-hosts/multica.md)
- [架构决策](adr/README.md)
