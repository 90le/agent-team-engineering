...
v0.8.1提供从v0.2、v0.3、v0.4、v0.5.0、v0.5.1、v0.6.0、v0.7.0、v0.8.0到v0.8.1的显式迁移：先在实例外输出摘要绑定计划，人工审阅后创建外部恢复包，再逐项迁移managed文件并最后提交目标锁。用户修改过的seeded文件原样保留。运行库存在时必须暂停、审计通过且无活动租约/未决外部效果；操作者还必须另外停止所有Writer进程。旧版本运行批准不会自动迁移为新版本批准。
