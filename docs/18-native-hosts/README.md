...
Each deterministic stage and random per-file intent is proposal-bound. A crash before `ACTIVE` resumes only from the exact intent inode; a crash after `ACTIVE` can finish deleting only intents whose binding equals the recorded target. The initial empty intent authorizes only the first metadata transition and grants no general delete authority. An exact empty guard without a lock may be reused after guard fsync, but is never deleted and grants no deletion authority. Verification rejects every leftover per-file/metadata/initial intent or stage.
