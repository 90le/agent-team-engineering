...
Review the projected files and deterministic file stages, the declared transient metadata stage `.agent-team/.host-lifecycle.json.stage`, the exact random empty apply-intent path `.agent-team/.host-apply.intent-<32-hex>`, the empty guard format/digest, expected prior guard/tombstone, all delete effects, retention, limitations, and digest before following the displayed `confirm`, `apply`, and `verify` instructions. Plan schema `1.1.0` and install-lock schema `1.1.0` bind the complete proposal; an unexecuted v0.9 plan must be rebuilt, previewed, and confirmed again. At planning, the metadata stage and every apply-intent-prefix entry must be absent. On first apply, projected files and their stages must be absent; after collision checks, apply creates only the plan-bound empty intent, durably writes the exact `APPLYING` record, removes that intent, and may then publish deterministic file stages. A crash may leave the exact intent and metadata stage; only the same confirmed plan may validate that empty intent and resume. Every other pre-existing or differently named intent/stage is preserved and rejected. Apply/uninstall require their declared transient files to be absent on success and never touch similarly named hidden files. `filesystem_deletes: true` discloses transient lifecycle cleanup, while `persistent_filesystem_deletes` is true only when the newly confirmed plan will remove its exact prior tombstone. An exact empty guard with no install lock may be reused for guard-fsync recovery, but it is never deleted and grants no file-deletion authority. Unrelated content is preserved; collisions, baseline drift, or a concurrent lifecycle operation fail closed. Apply does not log in, create credentials, bind channels, write to a Multica workspace, merge code, or deploy production.
